...
| `tools/verify_package.py` | Audits a package's whole checksum cascade before it is flashed. Deliberately does **not** parse the `*_ctrl.bin` record layout — `patch_media` will not trust a record stride it has not seen, and neither does this; it looks for each CRC as a value instead. |
